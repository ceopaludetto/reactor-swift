...
public class PublisherVerificationExecutor: Quick.SyncDSLUser {
	...
	private static func createTestContext<V: PublisherVerification>(
		...
		try self.createTestContextWithMultipleSubscribers(verification, demand, 1) { publisher, subscibers in
		...
		describe("ReactiveStreams TCK") {
			describe("required") {
				...
				it("createPublisher1MustProduceAStreamOfExactly1Element") {
					try self.createTestContext(verification, 1) { _, subscriber in
				...
				it("createPublisher3MustProduceAStreamOfExactly3Elements") {
					try self.createTestContext(verification, 3) { _, subscriber in
				...
				context("spec101") {
					it("subscriptionRequestMustResultInTheCorrectNumberOfProducedElements") {
						try self.createTestContext(verification, 5) { _, subscriber in
				...
				context("spec109") {
					...
					it("mustIssueOnSubscribeForNonNullSubscriber") {
						try self.createTestContext(verification, 0) { _, subscriber in
				...
				context("spec302") {
					it("mustAllowSynchronousRequestCallsFromOnNextAndOnSubscribe") {
						try self.createTestContext(verification, 6) { _, subscriber in
							subscriber.requestNext(1)
							subscriber.requestNext(1)
							subscriber.requestNext(1)
				...
				context("spec309") {
					it("requestZeroMustSignalIllegalArgumentException") {
						try self.createTestContext(verification, 10) { _, subscriber in
				...
				context("spec312") {
					xit("cancelMustMakeThePublisherToEventuallyStopSignaling") {
						try self.createTestContext(verification, 20) { _, subscriber in
							subscriber.requestNext(10)
				...
				context("spec317") {
					...
					it("mustNotSignalOnErrorWhenPendingAboveLongMaxValue") {
						try self.createTestContext(verification, .max) { _, subscriber in
			...
			describe("optional") {
				context("spec104") {
					it("mustSignalOnErrorWhenFails") {
						...
						try self.createFailedTestContext(verification) { _, subscriber in
				...
				context("spec111") {
					...
					it("mustProduceTheSameElementsInTheSameSequenceToAllOfItsSubscribersWhenRequestingOneByOne") {
						try self.createTestContextWithMultipleSubscribers(verification, 5, 3) {
							let subscriber1 = $1[0]
							let subscriber2 = $1[1]
							let subscriber3 = $1[2]
					...
					it("mustProduceTheSameElementsInTheSameSequenceToAllOfItsSubscribersWhenRequestingManyUpfront") {
						try self.createTestContextWithMultipleSubscribers(verification, 3, 3) {
							let subscriber1 = $1[0]
							let subscriber2 = $1[1]
							let subscriber3 = $1[2]
